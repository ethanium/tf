    ...
    <td height="399" valign="top"><p class="style18">
      <?php
		  ...
		  $db=mysql_pconnect("localhost","root","");
				...
				//nagrerewcord pero l s database
				$query="UPDATE tblogger SET password='$newpass' WHERE name=".'Password'.'$pno';
				
				$result= mysql_query($query);
				
				
			
				/*echo"<p> Number of records found:".$result."<p>";
				
				while($row=mysql_fetch_array($result))
				{$x=htmlspecialchars(stripslashes($row["password"]));
				echo"$x";
				}*/
				if(!$result)
					{	
					echo"<P>Could'nt update password , check your entries.";
					}
				else 
